...
class SSP {
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				if ( isset( $column['formatter'] ) ) {
                    ...
                    if(!empty($column['db'])){
                    	if ($columns[$j]['db'] == 'depict')
                    		$row[ $column['dt'] ] = $data[$i][ 'nickname' ];
                    	else
	...
	static function simple ( $request, $conn, $table, $primaryKey, $columns )
	{
		...
		$data = self::sql_exec( $db, $bindings,
			"SELECT `".implode("`, `", self::pluck($columns, 'db'))."`
			 , (SELECT v2 FROM `gr_options` WHERE type = 'profile' and v1 = 'name' and  v3 = gr_site_users.user_id) as nickname FROM `$table`
			 $where
			 $order
			 $limit"
		);
	...
	static function complex ( $request, $conn, $table, $primaryKey, $columns, $whereResult=null, $whereAll=null )
	{
		...
		//self::sql_exec( $db, 'SET NAMES utf8mb4 COLLATE utf8mb4_unicode_ci');
		// Build the SQL query string from the request
		$limit = self::limit( $request, $columns );
		$order = self::order( $request, $columns );
		$where = self::filter( $request, $columns, $bindings );
